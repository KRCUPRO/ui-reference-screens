# Registro de cambios

[English](../readme/README.en.md) | [한국어](../readme/README.ko.md) | [简体中文](../readme/README.zh.md) | [日本語](../readme/README.ja.md) | **Español** | [Français](../readme/README.fr.md) | [Русский](../readme/README.ru.md) | [العربية](../readme/README.ar.md) | [हिन्दी](../readme/README.hi.md) • [📜 Registro de cambios](../changelog/CHANGELOG.es.md)

---

## v1.0.0 — 2026-09-07

- Versión inicial.
- 248 pantallas publicadas en 11 áreas.
- Galería con vista previa al pasar el cursor, búsqueda por título, filtrado por viewport y navegación por áreas.
- Documentación en nueve idiomas.

---

[Volver al README principal](../readme/README.es.md)
